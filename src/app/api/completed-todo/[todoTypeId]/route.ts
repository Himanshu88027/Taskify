...
export async function POST(
  ...
  try {
    ...
    const { isCompleted, completeId, progress } = await request.json();
    const { todoTypeId } = params;


    // Input validation
    if (!todoTypeId || !completeId) {
      return new Response(
    ...
    await UserModel.updateOne(
      {
        ...
        "todoTypes.todoTasks._id": completeId,      // Find the correct todoTask
      },
      {
        $set: {
          "todoTypes.$[type].todoTasks.$[isCompleted].isCompleted": isCompleted, // Update the task field
          "todoTypes.$[type].todoTasks.$[isCompleted].progress": progress, // Update the task field
        },
      },
      {
        arrayFilters: [
          { "type._id": todoTypeId },       // Filter to match correct todoType
          { "isCompleted._id": completeId },               // Filter to match correct todoTask
        ],
      }
    );
